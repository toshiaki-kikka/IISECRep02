...
switch($command) {
	"status"
		{
			try {
				$service = Get-Service -Name $tomcat_service_name -ErrorAction Stop
				Write-Host "サービス [$tomcat_service_name] は登録されています。"
				Write-Host "状態: $($service.Status)"  # Running / Stopped / StartPending / StopPending など
			}
			catch {
				Write-Host "サービス [$tomcat_service_name] は登録されていません。"
			}
			break
		}
	"register"
